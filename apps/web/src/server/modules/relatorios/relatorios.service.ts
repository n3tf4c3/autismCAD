...
export async function consolidateEvolutivoReport(params: {
  ...
  const indicadores = {
    ...
    feriados: atend.filter((a) => a.presenca === "Feriado").length,
    recessos: atend.filter((a) => a.presenca === "Recesso").length,
  ...
  let countDuracao = 0;
  atend.forEach((a) => {
    if (a.presenca !== "Feriado" && a.presenca !== "Recesso" && a.duracao_min > 0) {
  ...
  indicadores.mediaMinutosPorSessao = countDuracao
    ...
    : 0;
  const atendimentosAvaliaveis = indicadores.totalAtendimentos - indicadores.feriados - indicadores.recessos;
  indicadores.taxaPresencaPercent = atendimentosAvaliaveis
    ? Math.round((indicadores.presentes / atendimentosAvaliaveis) * 1000) / 10
    : 0;
  ...
  const distribuicao = {
    porPresenca: {
      ...
      Feriado: indicadores.feriados,
      Recesso: indicadores.recessos,
  ...
  if (tp >= 85 && atendimentosAvaliaveis >= 4) regras.push("ADESAO_BOA");
  if (indicadores.ausentes >= 3 || (atendimentosAvaliaveis > 0 && tp < 70)) regras.push("MUITAS_FALTAS");
  if (atendimentosAvaliaveis && indicadores.naoInformado / atendimentosAvaliaveis > 0.4) {
  ...
  const adesaoTexto =
    atendimentosAvaliaveis === 0
      ? "Sem atendimentos fora de feriados e recessos para avaliar adesão no período."
      : tp >= 85
...
export async function consolidateAssiduidadeReport(params: {
  ...
  const feriados = rows.filter((a) => a.presenca === "Feriado").length;
  const recessos = rows.filter((a) => a.presenca === "Recesso").length;
  ...
  rows.forEach((a) => {
    ...
    else if (a.presenca === "Feriado") item.feriados += 1;
    else if (a.presenca === "Recesso") item.recessos += 1;
    else item.neutros += 1;
